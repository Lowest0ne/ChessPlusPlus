...
int  Piece::type(void)       const {return (me & MASK_TYP ) >> SHFT_TYP;      }
int  Piece::color(void)      const {return (me & MASK_CLR ) >> SHFT_CLR;      }
bool Piece::moved(void)      const {return (me & MASK_MVD ) >> SHFT_MVD;      }
bool Piece::captured(void)   const {return (me & MASK_CPT ) >> SHFT_CPT;      }
int  Piece::xPos(void)       const {return (me & MASK_PSX ) >> SHFT_PSX;      }
int  Piece::yPos(void)       const {return (me & MASK_PSY ) >> SHFT_PSY;      }
bool Piece::checking(void)   const {return (me & MASK_CHK ) >> SHFT_CHK;      }
bool Piece::checked(void)    const {return (me & MASK_CKD ) >> SHFT_CKD;      }

int Piece::arrPos(void)const{return(xPos()+ (yPos()*WIDTH) );}
...
void Piece::king_checked(Piece* pieces, Piece* board[], std::bitset<BOARD_SIZE>& check_mask)
{
	...
	{
		enemy = &pieces[i];
		if (!enemy->moves()[this->arrPos()]) continue;
...
std::bitset<BOARD_SIZE> Piece::makeCheckMask(Piece* pieces, Piece* board[])
{
	...
	{
		if (!p->moves()[myKingArrPos]) continue;
...
std::bitset<BOARD_SIZE> Piece::moves(const Piece* pieceList)
{
...
std::bitset<BOARD_SIZE>& Piece::moves(void)
{
	return this->m_moves;
}
